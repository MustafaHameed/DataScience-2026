# Build the Urdu brief edition of the AIOU 437 Islamiat (Elective) course guide.
...
# This course sets Quranic and hadith text in Naskh; Nastaliq collides the
# harakat. Missing it would not fail the build, only silently degrade it.
if ($installed -notcontains 'Traditional Arabic') {
    Write-Host 'Font "Traditional Arabic" is not installed.' -ForegroundColor Red
    Write-Host 'It carries every Quranic and hadith quotation in this guide.' -ForegroundColor Red
    Write-Host 'Substitute another vowelled Naskh face (e.g. Amiri, Scheherazade)' -ForegroundColor Red
    Write-Host 'for \naskhfont in aiouurdu.sty.' -ForegroundColor Red
    exit 1
}
...
Write-Host ''
Write-Host '--- Build report (437 Urdu brief) ---' -ForegroundColor Cyan
