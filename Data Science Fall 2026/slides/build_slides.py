#!/usr/bin/env python3
"""Build the Data Science Fall 2026 lecture decks from the handout.

    python build_slides.py --ch 14         one chapter
    python build_slides.py --part 3        a whole part
    python build_slides.py --all           all 32 chapters + the overview
    python build_slides.py --overview      the course-overview deck only
...
PARTS = [(1, 6), (7, 10), (11, 16), (17, 22), (23, 27), (28, 32)]
...
def build_chapter(n: int, chapters, deps, figman, verbose=True) -> str:
    ...
    path = os.path.join(OUT, f"DS_Ch{n:02d}_{safe(ch.title)}.pptx")
...
def main() -> int:
    ...
    if a.part:
        ...
        want = [] if a.overview else [14]
    os.makedirs(OUT, exist_ok=True)
    for n in want:
        build_chapter(n, chapters, deps, figman)
    if a.overview or a.all:
        import overview
        overview.build(chapters, deps, figman)
    return 0
