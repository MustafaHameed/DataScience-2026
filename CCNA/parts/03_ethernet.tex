...
\section{Reading hexadecimal first}

Everything in this chapter --- and every MAC address, every IPv6 address and
every frame type for the rest of the book --- is written in \term{hexadecimal}.
It is worth two minutes now, because nothing later makes sense without it.

\begin{definitionbox}[Hexadecimal, and why it exists]
Ordinary decimal counts in tens using ten symbols, 0 to 9. \term{Hex} counts in
sixteens using sixteen symbols: 0 to 9, then \cmd{a b c d e f} for ten to
fifteen.

It exists because \textbf{one hex digit is exactly four bits}, so two hex digits
are exactly one byte. Writing a 48-bit MAC address in decimal would take
fifteen digits with no relationship to the underlying bits; in hex it takes
twelve, and each one maps to a nibble you can reason about.
\end{definitionbox}

% Nine columns: the budget is about 12.8 cm, not the 15.78 of two.
\begin{longtable}{@{}L{1.2cm}L{1.2cm}L{1.6cm}L{1.2cm}L{1.2cm}L{1.6cm}L{1.2cm}L{1.2cm}L{1.6cm}@{}}
\toprule
\rowcolor{primary!15}
\textbf{Hex} & \textbf{Dec} & \textbf{Binary} &
\textbf{Hex} & \textbf{Dec} & \textbf{Binary} &
\textbf{Hex} & \textbf{Dec} & \textbf{Binary} \\
\midrule
\endhead
\cmd{0} & 0 & \cmd{0000} & \cmd{6} & 6 & \cmd{0110} & \cmd{c} & 12 & \cmd{1100} \\
\rowcolor{lightbg}
\cmd{1} & 1 & \cmd{0001} & \cmd{7} & 7 & \cmd{0111} & \cmd{d} & 13 & \cmd{1101} \\
\cmd{2} & 2 & \cmd{0010} & \cmd{8} & 8 & \cmd{1000} & \cmd{e} & 14 & \cmd{1110} \\
\rowcolor{lightbg}
\cmd{3} & 3 & \cmd{0011} & \cmd{9} & 9 & \cmd{1001} & \cmd{f} & 15 & \cmd{1111} \\
\cmd{4} & 4 & \cmd{0100} & \cmd{a} & 10 & \cmd{1010} & & & \\
\rowcolor{lightbg}
\cmd{5} & 5 & \cmd{0101} & \cmd{b} & 11 & \cmd{1011} & & & \\
\bottomrule
\end{longtable}

\begin{conceptbox}[The three things you actually have to do with it]
\begin{tight}
  \item \textbf{Recognise it.} A leading \cmd{0x} means ``what follows is hex'' ---
        \cmd{0x0800} is the number 2048, not eight hundred. Cisco also writes hex
        with no prefix at all, as in \cmd{00d0.58a1.0201}.
  \item \textbf{Expand one digit to four bits}, using the table above. This is
        all that EUI-64 (\chref{ipv6}) and the MAC address bits below require.
  \item \textbf{Know that \cmd{f} is all ones.} So \cmd{ffff.ffff.ffff} is
        forty-eight 1 bits --- every bit set --- which is exactly why that is the
        broadcast address.
\end{tight}

You will never be asked to convert a long hex number to decimal in your head.
You will regularly be asked what one digit means in bits.
\end{conceptbox}

\begin{worked}[What is the first byte of \cmd{00d0.58a1.0201} in bits?]
The first byte is the first \emph{two} hex digits, \cmd{00}.

From the table, \cmd{0} is \cmd{0000}. So \cmd{00} is
\cmd{0000\,0000} --- eight zero bits.

That matters immediately: the conceptbox after the frame explains that the
low-order bit of the first byte says unicast or multicast, and the next bit up
says whether the address was assigned by a manufacturer. Both are zero here, so
this is a unicast address burned in by its vendor. In \chref{clienttshoot} you
will meet an address where the second bit is \emph{one}, and that single bit is
the whole diagnosis.
\end{worked}
...
\end{tikzpicture}}%
{The whole switching algorithm. Flooding is not a failure --- it is how the table
gets built, and the second frame between the same two hosts is already
efficient.}{switchflood}
