...
% The two marks, at matched optical height. \IfFileExists rather than a plain
% \includegraphics so the volume still builds for anyone who has the sources
% but not the artwork -- which is how it was written.
\IfFileExists{assets/iub-logo.png}{%
  ...
  {\footnotesize\color{gray!60!black}Department of Information Technology}\\[14pt]
}{}
...
\vspace{0.9cm}
{\small\color{gray!45!black}Dr. Mustafa Hameed}\\[3pt]
...
this way \\
\faIcon{calculator}~\textbf{Worked Example} & A calculation done in full, one
step at a time. Cover the answer and try it first; these are the places the exam
expects speed \\
...
\faIcon{terminal}~\textbf{Host command} & The same, but run on a Windows, macOS
or Linux machine rather than on IOS. The operating system is named in the title,
because \cmd{ipconfig} and \cmd{ip addr} are not interchangeable \\
\faIcon{file-code}~\textbf{Playbook} & An Ansible YAML file, named by its
filename. Chapters~\ref{ch:ansible} onward \\
...
\begin{tight}
  ...
  \item \textbf{Somewhere to write.} A test centre supplies an erasable
        whiteboard, not paper, and you may not bring your own --- so practise
        subnetting under the same constraint, and without a calculator.
\end{tight}
