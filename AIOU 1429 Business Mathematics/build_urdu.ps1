# Build an Urdu edition of the AIOU 1429 solved question bank.
#   .\build_urdu.ps1                 27-question edition (35 pp)
#   .\build_urdu.ps1 -Brief          16-question brief edition (<=21 pp)
#   .\build_urdu.ps1 -Clean          remove aux files first
#   .\build_urdu.ps1 -Brief -Clean   both
#
...
param([switch]$Clean, [switch]$Brief)

$ErrorActionPreference = 'Stop'
Set-Location $PSScriptRoot
$main = if ($Brief) { 'AIOU_1429_Urdu_Brief' } else { 'AIOU_1429_Urdu_Guess_Paper' }

# The brief edition is contractually capped at 21 pages; the whole point of it
# is that it fits in one sitting. Enforce that here rather than trusting a
# later eyeball.
$pageCap = if ($Brief) { 21 } else { 0 }
...
if ($installed -notcontains 'Urdu Typesetting') {
    ...
    Write-Host 'Urdu Naskh face in bmurdu.sty (e.g. Noto Nastaliq Urdu).' -ForegroundColor Red
    exit 1
}
...
Write-Host ''
Write-Host '--- Build report (Urdu edition) ---' -ForegroundColor Cyan
...
if ($missing -gt 0) {
    ...
    Write-Host 'not merely mis-shaped. Check the \setmainfont line in bmurdu.sty.' -ForegroundColor Red
    exit 1
}
...
if ($pageCap -gt 0 -and $pages -match '^\d+$' -and [int]$pages -gt $pageCap) {
    Write-Host ''
    Write-Host "Brief edition is $pages pages, over its $pageCap-page cap." -ForegroundColor Red
    Write-Host 'Cut a question or tighten a solution; do not raise the cap.' -ForegroundColor Red
    exit 1
}
