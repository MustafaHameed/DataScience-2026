#!/usr/bin/env python3
"""Read the Machine Learning handout into structured slide content.

Copied from ../../Data Science Fall 2026/slides/texparse.py and adapted:
master file, part titles and appendix file names. Keep the two copies in
step when either is improved.
...
ROOT = os.path.dirname(HERE)                      # ...\Machine Learning Fall 2026
PARTS = os.path.join(ROOT, "parts")
MASTER = os.path.join(ROOT, "ML_Fall2026_Handout.tex")
AUX = os.path.join(ROOT, "ML_Fall2026_Handout.aux")
CODE = os.path.join(ROOT, "code")

PART_TITLES = {
    1: "Foundations",
    2: "Linear Models and Generalization",
    3: "Supervised Learning Algorithms",
    4: "Learning Without Full Labels",
    5: "Sequences, Decisions and Reinforcement",
    ...
    while True:
        ...
        if m.group("env"):
            ...
            if env in BOX_ENVS:
                ...
                # one centred formula per line: the slide renderer sets
                # each MathE as a single $\displaystyle ...$
                for line in re.split(r"\\\\(?:\[[^\]]*\])?", inner):
                    if line.strip():
                        out.append(MathE(resolve_refs(line.strip())))
            elif env in ("align", "align*"):
                out.append(MathE(resolve_refs(
                    r"\begin{aligned}" + inner.strip() + r"\end{aligned}")))
            elif env in ("equation", "equation*"):
                out.append(MathE(resolve_refs(inner.strip())))
...
def chapter_order() -> list[tuple[int, str]]:
    """[(number, stem)] for the chapters, in master-file order."""
...
def _answers() -> dict[int, list[Para]]:
    raw = open(os.path.join(PARTS, "C_answers.tex"), encoding="utf-8").read()
    src, listings = _protect_listings(raw)
    src = strip_comments(src)
    ctx = _Ctx(listings)
    out = {}
...
def schedule() -> list[Week]:
    raw = open(os.path.join(PARTS, "F_schedule.tex"), encoding="utf-8").read()
...
def audit() -> int:
    ...
    for n, stem in chapter_order():
        ...
        # A tabular or display inside a figure's TikZ body is part of the
        # rendered figure, not a slide table or formula: count it there only.
        bare, pos = [], 0
        for m in re.finditer(r"\\dsfigh?\{", src):
            if m.start() < pos:
                continue
            _, end = find_group(src, m.end() - 1)
            bare.append(src[pos:m.start()])
            pos = end
        bare = "".join(bare) + src[pos:]
        want = {
            "figure": len(re.findall(r"\\dsfigh?\{", src)),
            "table": len(re.findall(r"\\begin\{(longtable|tabular)\}", bare)),
            "code": len(re.findall(r"\\begin\{lstlisting\}", raw)),
            # \[ ... \], equation and align are one formula each; gather
            # gives one per line (see parse_body)
            "math": len(re.findall(r"(?<!\\)\\\[", bare)) +
            len(re.findall(r"\\begin\{(equation|align)\*?\}", bare)) +
            sum(len([ln for ln in re.split(r"\\\\(?:\[[^\]]*\])?", g)
                     if ln.strip()])
                for g in re.findall(r"\\begin\{gather\*?\}(.*?)"
                                    r"\\end\{gather\*?\}", bare, re.S)),
    ...
    print(f"schedule: {len(wk)} weeks, chapters covered "
          f"{'all ' + str(len(chapter_order())) if covered == list(range(1, len(chapter_order()) + 1)) else covered}")
