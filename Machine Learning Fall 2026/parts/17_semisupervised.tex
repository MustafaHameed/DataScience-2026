...
Labels are expensive. A senior engineer must read a ticket to name its root
cause; a reviewer must study a commit to say whether it introduced a defect; a
project manager must reconstruct why a project slipped. Unlabelled examples are
nearly free: every ticket in the tracker, every commit in the history, every
...
\end{tightnum}
It works with any classifier that gives probabilities, and on the tickets of this
chapter's lab it lifts accuracy from 92\% with 50 labels to 97\% using the 1{,}210
unlabelled tickets as well (all 1{,}260 labels give 98\%). Its risk is
...
Six tickets took $x = 1, 2, 3, 7, 8, 9$ hours to resolve. There seem to be two kinds
--- quick fixes and investigations --- but nobody recorded which was which. Model:
two Gaussians with known $\sigma = 1$ and equal weights; unknown means. Start from
$\mu_1 = 2$, $\mu_2 = 5$.
...
used it to classify text documents from a handful of labelled ones and thousands of
unlabelled ones --- exactly the position of a ticket tracker with two hundred
triaged tickets and ten thousand untriaged. The recipe:
\begin{tightnum}
...
The six tickets of the previous example, but now an engineer has checked two: the
3-hour ticket was a quick fix (class A) and the 7-hour ticket an investigation
...
Part 3 uses 1{,}800 synthetic tickets, each described by ten numeric features of
its text and history, with four possible root causes; only 50 of the training
tickets are labelled.
