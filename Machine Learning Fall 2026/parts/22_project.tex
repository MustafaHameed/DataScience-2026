...
\begin{tight}
  \item the UCI Machine Learning Repository, OpenML and Kaggle, each with thousands of
        documented datasets;
  \item the datasets bundled with scikit-learn, for rehearsing the pipeline;
  \item data from your own institution or employer --- only with written
        permission, and never with personal identifiers (\chref{ethics}).
\end{tight}

\begin{worked}[From a Vague Idea to a Project]
\textbf{Idea:} ``Use machine learning to improve student results.''

\smallskip
\textbf{Decision.} Who acts on the prediction? The department's advisers, who can
contact about 50 students each term.

\textbf{Task $T$.} In Week~4, rank enrolled students by their risk of failing the
course.

\textbf{Experience $E$.} Three past cohorts (about 900 students) with Week-4 attendance,
quiz marks and LMS activity, and final results --- obtained anonymised, with the
department's written permission.

\textbf{Performance $P$.} Recall among the top 50 ranked students (how many eventual
failures the advisers would reach), with precision reported beside it.

\textbf{Baseline.} Rank by attendance alone --- the rule advisers already use.

\textbf{Success criterion.} Beat the attendance rule's recall at 50 by at least five
students on the held-out cohort, with no group of students (by programme) whose recall
is below 60\% of the others'.
...
The lab's template writes this log for a tumour classifier whose metric is recall on
the malignant class:
...
logistic regression & 0.944 & 0.036 \\
random forest & 0.937 & 0.034 \\
\bottomrule
\end{tabular}
\end{center}

\smallskip
\textbf{The baseline scores zero}: it predicts ``benign'' for everyone, so it never
catches a malignant case --- the accuracy paradox of \chref{evaluation} expressed in the
metric that matters. \textbf{The two models} --- logistic regression (\chref{logreg})
and a random forest (\chref{ensembles}) --- \textbf{are indistinguishable}: 0.944 and 0.937
differ by far less than one fold-to-fold standard deviation. Choose the simpler, more
explainable one --- logistic regression --- tune it (the template finds $C = 0.1$ with
balanced class weights), and only then open the test set, once: recall 0.962 on 53
malignant cases, which is 51 of 53 caught.
\end{worked}
...
%---------------------------------------------------------------------
\begin{fourlenses}{Project Ideas}
\lensLA{Predict course failure from early-term engagement (the worked example); cluster
students by study behaviour and describe each group; or run a bandit over reminder
messages and report which works best. Obtain anonymised data with written permission,
and include the fairness check across programmes.}

\lensPM{Estimate issue resolution time from ticket text and metadata (regression plus
naive Bayes features); or classify projects at risk from their weekly metrics. Open
issue trackers from public software projects are an accessible, legal source.}

\lensIOT{Detect machine faults from a public vibration or predictive-maintenance
dataset; compare an anomaly detector trained on normal data with a supervised
classifier, and report the cost of each kind of error.}

\lensSEC{Classify network connections as benign or malicious on a public
intrusion-detection dataset, or phishing URLs from their features. Report precision at
fixed recall, compare with a signature-style rule, and test how easily the model can be
evaded by changing one feature.}
\end{fourlenses}
...
\begin{checkpoint}
\begin{tightnum}
  \item Write a success criterion for a spam filter that a manager could check.
...
\begin{reviewq}
  ...
  \item \textbf{(Short)} Explain why the worked example chose logistic regression over
        the random forest.
  \item \textbf{(Applied)} Write the one-page proposal for a project in a domain of your
        choice, using the template of this chapter.
  \item \textbf{(Applied)} For the worked example's test result (51 of 53 malignant
        cases caught), compute a 95\% confidence interval for the recall and discuss
        what it means for the claim ``the model catches 96\% of cancers''.
\end{reviewq}
