...
\dsref{polyfits} fits polynomials of three degrees to the same ten noisy samples
of a sine wave. Degree 1 is too rigid to follow the curve at all. Degree 9 has ten
...
noise: a student recorded as failing who in fact withdrew, an intrusion alert
closed as benign by a tired analyst. Noise sets the floor on achievable error,
...
%---------------------------------------------------------------------
\begin{fourlenses}{Bias and Variance}
\lensLA{A dropout model trained on one cohort of 200 students is a high-variance
model: retrained on next year's cohort it gives different students different
risks. Strong regularisation and a small number of well-understood features
usually beat a flexible model here, because the data is small and the cohorts
drift.}

\lensPM{Effort data from one team is tiny and noisy, so the right model is nearly
always simpler than it looks. A regression with two features that generalises
beats one with twenty that fits last quarter perfectly; learning curves on
project data usually show high variance.}

\lensIOT{Vibration data is plentiful but each machine is different. A model with
low variance across machines --- tested by holding out whole machines in
cross-validation, not random readings --- is worth more than one that is very
accurate on the machines it was trained on.}

\lensSEC{An intrusion model that fits last month's traffic perfectly has learned
last month's attacks, including their noise. Overfitting here is also a security
weakness: a model that memorises exact signatures is evaded by the smallest
variation. Regularised, simpler models degrade more gracefully.}
\end{fourlenses}
