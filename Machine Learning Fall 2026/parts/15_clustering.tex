...
examine. Kinds of project in a portfolio, tickets that describe the same
underlying fault, developers with similar skill profiles, the phases a project
passes through --- each is a clustering that someone then named and decided was
useful. Two families of algorithm cover the HEC outline:
...
Seven past projects, each described by its budget (in hundreds of thousands of
dollars) and its duration (in months): A1 $(1, 1)$, A2 $(1.5, 2)$, A3 $(3, 4)$,
A4 $(5, 7)$, A5 $(3.5, 5)$, A6 $(4.5, 5)$, A7 $(3.5, 4.5)$. $k = 2$, with initial
centroids A2 and A4.
...
two small, short projects and five larger ones.
\end{worked}
...
Five tickets, described only by how many days each took to resolve: 1, 2, 4, 9, 11.
...
\end{lstlisting}
Part 2 stands in for 600 projects of four kinds, described by two numeric
features; the kinds are known (\texttt{truth}) only so that part 4 can check the
clusters against them. The warning filter hides a harmless message scikit-learn
prints on some Windows installations. Change \texttt{cluster\_std} to 2.5 in part 2: do the elbow and the
