...
model will do on the next student, e-mail or sensor reading.
\end{conceptbox}
...
\midrule
\endhead
Numeric, continuous & Attendance rate, CPU load, bytes sent & Impute gaps; scale
for distance- and gradient-based models \\
Numeric, count & Logins per week, failed login attempts & As above; a log
transform often helps with long tails \\
Categorical, nominal & Programme, protocol, device type & One-hot encode; never
treat the codes as numbers \\
Categorical, ordinal & Grade band, severity level & Map to ordered integers, or
one-hot if the gaps are not equal \\
Text & E-mail body, commit message & Turn into counts or weights of words
(\chref{bayes}) \\
Identifier & Student ID, IP address, serial number & Usually drop. An ID that
...
per person, machine or account, keep all of an entity's rows on the same side of
...
missing \emph{for a reason} --- a sensor that fails when it overheats, a student
who never opened the quiz --- add a yes/no column recording that it was missing;
the absence is itself informative.

\textbf{Categories.} \term{One-hot encoding} replaces a categorical feature with
one 0/1 column per category. \texttt{programme} $\in \{$BSIT, BSCS, BSSE$\}$
becomes three columns, exactly one of which is 1 in each row. Coding the
programmes as 1, 2 and 3 instead would tell the model that BSSE is ``three times''
BSIT, which is nonsense that a linear model will happily use.
...
Four training students have attendance $0.6, 0.8, 0.9, 0.7$ and logins
$2, 4, 6, 8$.

\smallskip
\textbf{Attendance:} $\mu = 0.75$;
...
\smallskip
A new student arrives with attendance $0.5$ and 10 logins:
\[
z_{\text{att}} = \frac{0.5 - 0.75}{0.112} = -2.24, \qquad
z_{\text{log}} = \frac{10 - 5}{2.236} = +2.24
\]
\textbf{Both are now equally unusual} --- 2.24 standard deviations from the
training mean, in opposite directions --- where before the logins difference
(5) was twenty times the attendance difference (0.25). Note that $\mu$ and
$\sigma$ came from the \emph{training} students only. The new student is
transformed with the training statistics, never with statistics that include
the new student.
\end{worked}
...
after, or because of, the outcome: \texttt{days\_until\_account\_closed} in a model
of which customers will leave. \term{Train--test contamination} is subtler:
...
Baselines are humbling. On the breast-cancer data of \chref{introduction}'s lab,
``always benign'' already scores 63\%. On intrusion data where 99\% of connections
are benign, ``always benign'' scores 99\% --- which is why a detector's accuracy
means nothing until it is set beside the baseline (\chref{evaluation}).
...
\begin{tight}
  ...
  \item \textbf{Label-encoding a nominal feature.} Programme codes 1, 2, 3 impose an
        order that does not exist. One-hot encode.
  \item \textbf{Keeping an identifier.} If \texttt{student\_id} improves accuracy,
...
%---------------------------------------------------------------------
\begin{fourlenses}{Preparing the Data}
\lensLA{Missing quiz marks are not random: the students who skip quizzes are the
ones at risk. Impute, but add a \texttt{quiz\_missing} column, and it will often
be one of the strongest features in the model. Split by student, never by row,
if a student has several records.}

\lensPM{Project data is small and full of categories --- team, project type,
technology stack. One-hot encoding thirty technologies across two hundred
projects gives most columns only a handful of ones; group the rare categories
into ``other'' before encoding.}

\lensIOT{Sensor data is ordered in time, so split by time: train on January to
September, validate on October, test on November. A random split puts readings
from the same minute on both sides, and the test score measures interpolation,
not forecasting.}

\lensSEC{The classic security leak is a feature derived from the analyst's
verdict --- a ``ticket escalated'' flag, a signature name --- that exists only
because the event was already known to be malicious. Ask of every feature: would
this value exist, with this value, at the moment the model has to decide?}
\end{fourlenses}

%---------------------------------------------------------------------
\begin{lab}[A Leak-Proof Pipeline]
A synthetic cohort of 600 students with a missing-value problem and one
categorical feature. The whole workflow, in the order of \dsref{workflow}.
...
\begin{tightnum}
  \item Why must the scaler's mean and standard deviation be computed from the
        training set only?
  \item A dataset has a column \texttt{protocol} with values TCP, UDP and ICMP.
        How should it be encoded, and why not as 1, 2, 3?
  \item A model predicting equipment failure scores 97\% accuracy. What single
        number do you ask for before you are impressed?
\end{tightnum}
\end{checkpoint}
...
\begin{reviewq}
  ...
  \item \textbf{(MCQ)} A column recording the date a loan was written off, in a
        model predicting default, is an example of: (a)~target leakage
        (b)~stratification (c)~imputation (d)~a baseline.
