...
the other: a defective release shipped against a reviewer's hour spent on a
change that was fine, a late task nobody warned about against a needless check-in
with a team that was on track. A single number that adds the
...
\node[font=\scriptsize\bfseries, text=primary] at (1.55,2.0) {truly defective};
...
\node[cell, draw=accent, fill=accent!10] at (4.65,0.8)
     {{\color{accent}\bfseries FP = 75}\\[2pt]{\tiny needless review}};
...
\node[font=\tiny, text=gray!55!black] at (1.55,-1.8) {250 defective};
...
\node[anchor=north west] at (7.9,2.2) {%
  ...
  \end{tabular}};
\end{tikzpicture}}%
{A defect predictor's confusion matrix on 5{,}000 code changes, and the five
metrics computed from it. Precision reads along the ``flagged'' row; recall reads
down the ``truly defective'' column.}{confusion}

\begin{worked}[One Defect Predictor, Five Metrics]
Before a release, the predictor of \dsref{confusion} examined 5{,}000 code changes,
of which 250 later turned out to have introduced a defect. It flagged 300 for extra
review, of which 225 were defective.
...
changes: 95.0\% accuracy. The predictor's 98.0\% is three points better than
doing nothing, which sounds small; its recall of 90\% --- nine defective changes
in ten caught, at the cost of one needless review for every three real finds ---
...
For the defect predictor, a missed defective change (FN) is estimated to cost 50
times as much as a needless review (FP) --- a production incident against twenty
minutes of a reviewer's time. The predictor's precision and recall at four
thresholds, with 250 defective changes:
...
default 0.5 by accepting 121 more needless reviews to catch 15 more defects. With
...
rare-event detector. For defect, outage and overrun prediction, report the PR
...
A ticket router is tested on 100 tickets: 80 bug reports, 15 feature requests and
5 security reports. It labels 76 bug reports, 9 feature requests and 1 security
report correctly.

\smallskip
\textbf{Per-class recall:} bug $76/80 = 0.95$; feature $9/15 = 0.60$;
security $1/5 = 0.20$.
...
\smallskip
The micro figure is carried by the bug class; the macro figure reveals that the
router almost never recognises a security report. If the rare classes matter ---
and security reports are exactly the tickets that must not wait --- report the
macro figure and the per-class table.
\end{worked}
...
\begin{worked}[McNemar on 500 Test Tickets]
The current ticket router A and a new router B are run on the same 500 tickets.
They disagree on 40: B is right and A wrong on $b = 12$; A is right and B wrong on
$c = 28$.
\[
...
16 more tickets, 3.2 points of accuracy. The 460 tickets on which they agree play
...
Six thousand synthetic code changes, 5\% of which introduced a defect, each
described by twelve numeric features (size, files touched, the author's
...
\begin{checkpoint}
\begin{tightnum}
  \item Of 400 tasks flagged as at risk, 100 did finish late; 20 late tasks were
        not flagged. Compute precision and recall.
...
\begin{reviewq}
  ...
  \item \textbf{(Applied)} A defect predictor tested on 10{,}000 code changes, 200
        of them defective, flags 500, of which 160 are defective. Build the
        confusion matrix and compute accuracy, precision, recall, specificity and
        $F_1$. Which two numbers would you put in a report to the release
        manager?
