...
Code a binary label as $y = 1$ (fail) or $y = 0$ (pass), fit a least-squares line,
and call anything above 0.5 a fail. It half works --- and it breaks in two ways,
both visible on the left of \dsref{linear-vs-logistic}. The line predicts values
below 0 and above 1, which cannot be probabilities. And a few examples far from
the boundary --- a student with perfect attendance, who of course passed ---
drag the line towards them and move the 0.5 crossing, misclassifying students
near the boundary who were previously correct. The model is being penalised for
...
\begin{groupplot}[
  ...
  xlabel={risk score $x$}, ylabel={$P(\text{fail})$ or prediction},
...
\end{groupplot}
\end{tikzpicture}}%
{The same pass (green) and fail (red) students modelled two ways. The straight line
...
\begin{worked}[Probabilities and Odds for Two Students]
A fitted model of failing: $z = 4 - 5\,a - 3\,q$, where $a$ is attendance and $q$
the quiz average, both on a 0--1 scale.

\smallskip
\textbf{Student A} ($a = 0.9$, $q = 0.8$):
$z = 4 - 4.5 - 2.4 = -2.9$, so
$P(\text{fail}) = \dfrac{1}{1 + e^{2.9}} = \dfrac{1}{1 + 18.17} = \mathbf{0.052}$.

\textbf{Student B} ($a = 0.5$, $q = 0.4$):
$z = 4 - 2.5 - 1.2 = 0.3$, so
$P(\text{fail}) = \dfrac{1}{1 + e^{-0.3}} = \dfrac{1}{1 + 0.741} = \mathbf{0.574}$.

\smallskip
\textbf{Reading the attendance weight.} $w_a = -5$ per unit of attendance, so ten
percentage points more attendance ($+0.1$) changes the log-odds by $-0.5$ and
multiplies the odds of failing by $e^{-0.5} = \mathbf{0.61}$ --- a 39\% reduction in
the odds, whatever the student's starting point. Note: the \emph{odds}, not the
probability. For Student A the probability barely moves; for Student B, near the
middle of the curve, it moves a lot.
\end{worked}
...
\begin{axis}[width=8.2cm, height=6.4cm, axis lines=left,
  xlabel={attendance $a$}, ylabel={quiz average $q$},
...
\node[font=\tiny, primary!70, anchor=west, fill=white, inner sep=1pt] at (axis cs:0.21,0.66)
     {$P(\text{fail}) = 0.75$};
\node[font=\tiny\bfseries, accent, anchor=west] at (axis cs:0.22,0.08) {predicted fail};
\node[font=\tiny\bfseries, greenhl!55!black, anchor=east] at (axis cs:0.99,0.95) {predicted pass};
\end{axis}
\end{tikzpicture}}%
{The decision boundary of the model $z = 4 - 5a - 3q$ is the line $5a + 3q = 4$. The
dashed lines, where $z = \pm\ln 3$, are where the probability of failing is 0.75 and
0.25: probability changes gradually across the boundary.}{logreg-boundary}
...
A network-traffic classifier gives scores $z = (2.0,\ 1.0,\ 0.1)$ for the classes
\emph{web}, \emph{e-mail} and \emph{streaming}.
\[
...
%---------------------------------------------------------------------
\begin{fourlenses}{Logistic Regression}
\lensLA{The standard early-warning model. The probability ranks students for
outreach, and the odds ratios --- ``each missed quiz multiplies the odds of
failing by 1.4'' --- are something an academic committee will accept, which a
more accurate but opaque model may not be.}

\lensPM{Predict whether a project will overrun its deadline from team size,
requirement changes and dependencies. With a few hundred projects, logistic
regression is often as accurate as anything more complex, and its odds ratios
tell a portfolio manager which risk factor to act on first.}

\lensIOT{Predict whether a component fails within 30 days from its sensor
summaries. On a microcontroller, a trained logistic model is one dot product and
one exponential --- a few microseconds and a few hundred bytes, which is why it
is a favourite for on-device inference.}

\lensSEC{Phishing and spam filters were logistic regression over word and URL
features for years, and many still are. Its weakness is its linearity: an
attacker who learns which words carry negative weight can add them to a phishing
e-mail until the score drops below the threshold.}
\end{fourlenses}
...
\begin{checkpoint}
\begin{tightnum}
  \item A model gives $z = 1.2$ for an e-mail. What is the probability that it is
        phishing, and what are the odds?
