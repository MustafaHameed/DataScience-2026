...
Chapters 1, 4, 9, 10, 11, 13 of this handout follow its treatment; the source of
EnjoySport and PlayTennis \\
...
\midrule
\endhead
scikit-learn built-ins & \texttt{load\_breast\_cancer}, \texttt{load\_diabetes},
\texttt{load\_digits}, \texttt{load\_iris}; the \texttt{make\_*} generators. Offline;
used throughout the labs \\
UCI Machine Learning Repository & The classic collection; hundreds of documented
datasets \\
OpenML & Datasets with standard splits and benchmark results; loadable with
\texttt{sklearn.datasets.fetch\_openml} \\
Kaggle & Datasets with community notebooks --- read other people's work critically \\
Your institution & Often the most meaningful data; requires written permission and
anonymisation (\chref{ethics}) \\
\bottomrule
\end{longtable}
