...
\begin{worked}[One Student as a Vector]
A student has attendance $0.9$, quiz average $0.6$ and $4$ logins per week:
...
\smallskip
\textbf{Distance to a second student} $\mathbf{z} = (0.5,\ 0.4,\ 1)$:
\[
...
is logins. Attendance and quiz marks --- arguably the more important differences
--- contribute almost nothing, simply because they are measured on a scale of 0
to 1 and logins are not. Any method built on distance (\chref{knn},
...
Sometimes length is a nuisance. Two documents on the same topic, one ten times
longer, have word-count vectors pointing the same way but of very different
...
\node[font=\tiny, text=gray!55!black, anchor=east] at (-0.55,0) {student 1};
\node[font=\tiny, text=gray!55!black, anchor=east] at (-0.55,-0.5) {student 2};
\node[font=\tiny, text=gray!55!black, anchor=east] at (-0.55,-1.0) {student 3};
...
\begin{worked}[An Alarm You Should Not Panic About]
An intrusion detector catches 99\% of attacks and raises a false alarm on 1\% of
benign connections. One connection in a thousand is an attack. An alarm sounds.
What is the probability of an attack?

\smallskip
Let $A$ = attack, $B$ = alarm. $P(A) = 0.001$, $P(B\mid A) = 0.99$,
...
\]
\textbf{Only 9\% of alarms are attacks}, from a detector that is ``99\%
accurate''. There are so many more benign connections that their 1\% of false
alarms outnumbers the real attacks ten to one. This is the \term{base-rate}
...
\begin{axis}[width=8.4cm, height=4.6cm, axis lines=left,
  xlabel={$p$, the probability of heads}, ylabel={likelihood $p^7(1-p)^3$},
...
\node[font=\tiny, gray!55!black, anchor=west, align=left] at (axis cs:0.03,0.0019)
     {7 heads, 3 tails:\\$p=0.5$ explains the data\\less well than $p=0.7$};
\end{axis}
\end{tikzpicture}}%
{The likelihood of seven heads in ten tosses, as a function of the coin's bias. It is
largest at $p=0.7$ --- the observed proportion, which is what maximum likelihood gives for
any coin.}{likelihood}

\begin{worked}[The Maximum Likelihood Coin]
A coin gives 7 heads in 10 tosses. The likelihood of that sequence is
$\mathcal{L}(p) = p^7(1-p)^3$, and the log-likelihood is
...
One toss, one head: $\hat{p}_{ML} = 1$, a coin that can never land tails. A
word that never appeared in the training e-mails gets probability zero, and a
single zero makes a whole product zero. \chref{bayes} fixes this with
...
Fourteen days, of which 9 were suitable for tennis and 5 were not:
\[
...
\begin{tight}
  ...
  \item \textbf{Confusing $P(A\mid B)$ with $P(B \mid A)$.} ``The detector
        catches 99\% of attacks'' is $P(\text{alarm}\mid\text{attack})$; the
        analyst wants $P(\text{attack}\mid\text{alarm})$, which was 9\%.
...
%---------------------------------------------------------------------
\begin{fourlenses}{The Mathematics at Work}
\lensLA{A student is a vector of engagement features, and ``students like this
one'' means ``students at a small distance'' --- once attendance, marks and
logins have been put on the same scale. Without that, the student who logs in
most is automatically the most different from everyone.}

\lensPM{Fitting an effort model to past stories is gradient descent on the mean
squared error of the estimates. With forty completed stories the loss surface
is bumpy and the learning rate matters: too large and the fitted
hours-per-point swings wildly from one run to the next.}

\lensIOT{Sensor noise is usually close to Gaussian, so a reading three standard
deviations from its recent mean has a likelihood below one in three hundred
under ``normal operation''. That is the simplest anomaly detector there is, and
it is a likelihood calculation.}

\lensSEC{The base-rate calculation is the most important piece of arithmetic in
security analytics. Before deploying any detector, multiply its false-positive
rate by the daily volume of benign events: that number of alerts, not the
detection rate, decides whether the analysts will trust it.}
\end{fourlenses}
...
\end{lstlisting}
Change \texttt{0.001} in the simulation to \texttt{0.05} --- an environment where
attacks are common --- and predict the new answer before you run it.
\end{lab}
...
\begin{reviewq}
  ...
  \item \textbf{(MCQ)} The entropy of a fair six-sided die is: (a)~1 bit
  ...
  \item \textbf{(Applied)} A fraud model flags 2\% of legitimate transactions
        and 95\% of fraudulent ones; 0.2\% of transactions are fraudulent.
        Compute the probability that a flagged transaction is fraud, and state
        what this means for the team reviewing flags.
