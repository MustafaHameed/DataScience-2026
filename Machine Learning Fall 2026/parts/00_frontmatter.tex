...
{\small\itshape\color{gray!70!black}Every chapter shows its algorithms at work in all four domains.}
...
\faIcon{compass}~\textbf{Four Lenses} & The chapter's algorithm applied in learning analytics, project management, IoT and cybersecurity. \\
...
\begin{tightnum}
  ...
  \item \textbf{Use the Four Lenses panels to revise.} Being able to explain an
        algorithm in four different domains is strong evidence you understand it.
\end{tightnum}
...
societal problems & A3 Valuing & 21, and every Four Lenses panel \\
\bottomrule
\end{longtable}
...
Candidate elimination algorithm & 4 & Full trace on the \emph{EnjoySport} data;
inductive bias \\
...
Four application domains recur in every chapter of this handout, as they do in
the department's Data Science course. They were chosen because between them they
cover the situations a BSIT graduate actually meets, and because each one breaks
a learning algorithm in a different way.

\begin{fourlenses}{The Domains Themselves}
\lensLA{\textbf{Learning analytics} applies machine learning to teaching and
learning: predicting which students are at risk of failing, grouping learners by
how they study, recommending the next exercise. Its defining difficulty is that
the data describes \emph{people}, so fairness, privacy and the risk of a
self-fulfilling prophecy are never far away.}

\lensPM{\textbf{IT project management} applies machine learning to how work gets
delivered: estimating effort, forecasting whether a sprint will finish, flagging
projects drifting towards failure. Its defining difficulty is small, noisy,
human-reported data --- often a few hundred rows --- and the fact that a
prediction changes the behaviour of the team that hears it.}

\lensIOT{\textbf{The Internet of Things} applies machine learning to sensor
telemetry: detecting a failing motor from its vibration, forecasting energy
demand, deciding what can be computed on a small device. Its defining
difficulties are volume, drifting and missing sensors, rare failures, and a hard
limit on memory and computation at the edge.}

\lensSEC{\textbf{Cybersecurity} applies machine learning to defending systems:
classifying malware, spotting an intrusion in network traffic, noticing that an
account is behaving unlike itself. Its defining difficulties are extreme class
imbalance --- attacks are rare --- and a genuine \emph{adversary} who adapts to
whatever model you deploy.}
\end{fourlenses}

\begin{conceptbox}[Why Four Domains and Not One]
An algorithm you have only seen on one dataset is an algorithm you have
memorised. An algorithm you can transfer to a new setting is one you understand.
The Four Lenses panels force that transfer on every topic --- and the contrast is
instructive in itself. The same $k$-means that is merely convenient for grouping
students is a maintenance tool on a factory floor and an attack surface on a
firewall.
\end{conceptbox}
