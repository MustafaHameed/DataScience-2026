...
has $R^2 = 0$. On the diabetes data of this chapter's lab it has an RMSE of 70;
linear regression brings that to 56. Whether that improvement is worth having is
...
%---------------------------------------------------------------------
\begin{fourlenses}{Linear Regression}
\lensLA{Predict a final mark from coursework. The weights are the attraction:
``each extra lab submitted is worth 1.8 marks'' is something an adviser can say to
a student. The danger is reading it causally --- students who submit labs differ
in many ways from those who do not.}

\lensPM{Effort estimation is the textbook regression problem, and the
story-point example is real practice. Report RMSE in hours, compare with ``hours
per point'' from last quarter as the baseline, and refit every few sprints: teams
change, and so does the slope.}

\lensIOT{Predict energy use from outdoor temperature and occupancy. The residual
plot matters: consumption is U-shaped in temperature (heating when cold, cooling
when hot), so a straight line leaves exactly the pattern of
\dsref{residual-plots}; a squared temperature feature removes it.}

\lensSEC{Predict normal outbound traffic per host from time of day and number of
users, then treat a large positive residual as suspicious. The regression is not
the detector; its residuals are. This is regression used for anomaly detection,
and it works only if the training period was clean.}
\end{fourlenses}

%---------------------------------------------------------------------
\begin{lab}[Least Squares Three Ways]
The worked example by formula, then by gradient descent, then a real dataset:
disease progression in 442 diabetes patients from ten measurements.
