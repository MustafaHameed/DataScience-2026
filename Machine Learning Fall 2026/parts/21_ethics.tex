...
``discuss ethical considerations while applying ML to societal problems''; every
Four Lenses panel in the handout has been practice for it.
\end{prereq}
...
A model is a decision procedure applied at scale. A biased admissions officer affects
the applicants they meet; a biased admissions model affects every applicant, the same
...
\node[bi, anchor=north] at (5.6,-0.6) {\textbf{measurement}: the label is a proxy (arrests for crime, clicks for interest)};
...
discriminate on it. Other features act as \term{proxy variables}: home district,
school, name, even the time of day someone applies. In this chapter's lab, three
ordinary features recover group membership with 97\% accuracy. Second,
\term{feedback loops}: a model that sends more patrols to one district records more
incidents there, which becomes next year's training data, which sends more patrols.
The model's prediction makes itself come true.
...
\begin{worked}[Auditing an Admissions Model]
A model screens 1{,}000 applicants from each of two groups. Group A has 400 truly
qualified applicants; group B, whose members had fewer chances to build a strong record,
has 200.
...
%---------------------------------------------------------------------
\begin{fourlenses}{Responsible Machine Learning}
\lensLA{An early-warning model that flags students from one region more often may be
measuring unequal preparation, not ability; and a flag seen by a teacher can lower
expectations and cause the failure it predicted. Use flags to offer support, never to
restrict opportunity, and audit TPR by group every term.}

\lensPM{Models that score employees' productivity from commit counts or ticket
throughput measure what is easy to count. They reward gaming, and penalise the people
doing the unglamorous work --- reviews, mentoring, documentation --- that keeps a team
functioning.}

\lensIOT{Sensor data from homes and vehicles reveals when people are in, asleep or
travelling. Process on the device where possible, send aggregates rather than raw
streams, and treat a smart-meter dataset as the personal data it is.}

\lensSEC{Security models are the most adversarial setting of all: the people they
classify are actively trying to defeat them. Test against evasion and poisoning, and
remember that a false positive may lock an innocent user out of an account --- give
them a fast route back in.}
\end{fourlenses}

%---------------------------------------------------------------------
\begin{lab}[Auditing a Model for Fairness]
A synthetic admissions dataset in which group B had fewer chances to build a strong
...
impossibility result in action. Write two sentences, as if for the admissions committee,
...
\begin{reviewq}
  ...
  \item \textbf{(MCQ)} Using arrest records as the label for ``committed a crime'' is an
        example of: (a)~representation bias (b)~measurement bias (c)~a feedback loop
        only (d)~data poisoning.
  ...
  \item \textbf{(Short)} Explain a feedback loop with an example from a domain of your
        choice.
