...
Networks are the right tool when the input is high-dimensional and raw --- pixels,
audio samples, sensor streams --- when the target function is complex and unknown,
when some noise is acceptable, when long training times are acceptable, and when
nobody needs to read the learned function. Mitchell's examples were steering a car
from camera images and recognising faces; today's are the whole of deep learning.
...
Flips, crops and noise for images and signals \\
\bottomrule
\end{longtable}
...
\lensLA{Clickstream sequences from a learning platform --- which video, which
quiz, how long --- are raw, high-dimensional input where networks can find patterns
hand-built features miss. They need thousands of students, not hundreds; with one
course's data, the regularised linear model usually wins.}

\lensPM{Tabular project data is small and structured, the setting where networks
have the least advantage over trees and linear models. Where they help is text:
embedding ticket descriptions or commit messages before any other model sees
them.}

\lensIOT{Networks trained on raw vibration or audio windows detect faults that
summary statistics miss, and small networks can be compressed to run on
microcontrollers. The training cost is paid once, in the cloud; inference is a few
matrix multiplications on the device.}

\lensSEC{Networks classify malware from raw bytes and traffic from packet
sequences. They are also the most vulnerable to adversarial examples: tiny,
deliberately crafted input changes that flip the output. A security model is
judged by how it fails under attack, not only by its accuracy.}
\end{fourlenses}
...
\end{lstlisting}
The network learns XOR, and on the digits it beats logistic regression by less than a
point --- more with stronger weight decay. Now add \texttt{early\_stopping=True} to the
network and explain why accuracy \emph{falls} on this small dataset.
\end{lab}
