...
\begin{ans}
  \item No. The threshold was chosen by a person and the tool's performance does not
        improve with experience: there is no $E$. It is automation, not learning.
...
\begin{ans}
  ...
  \item One-hot encode it into three 0/1 columns. Codes 1, 2, 3 would tell a model that
        Waterfall is ``three times'' Scrum and that Kanban lies between them --- an
        order and distances that do not exist.
  \item The majority-class baseline: if 97\% of projects are never cancelled,
        predicting ``not cancelled'' is 97\% accurate. Then ask for recall on the
        cancelled projects.
\end{ans}

\ansch{4 --- Concept Learning and the Version Space}
\begin{ans}
  \item Yes: any release satisfying $\langle$Pass, Done, ?, Ready, ?, ?$\rangle$ also
        satisfies $\langle$Pass, ?, ?, Ready, ?, ?$\rangle$. The reverse is false: a
        release whose tests pass and whose rollback is ready, but whose review is
        still pending, satisfies the first but not the second.
  \item $\langle$Pass, Done, ?, Ready, Quiet, Same$\rangle$ --- the hypothesis after
        Release 2, since Release 3 is negative and ignored.
...
\begin{ans}
  \item The parent has entropy 1. Each child has entropy
        ...
        Gain $= 1 - 0.811 = 0.189$.
  \item Every ID is unique, so splitting on it gives one pure leaf per task: maximum
...
\begin{ans}
  \item Would break: $0.9 \times 0.01 = 0.009$; good: $0.05 \times 0.99 = 0.0495$. So
        $h_{MAP}$ is \emph{good}; $h_{ML}$ is \emph{would break} ($0.9 > 0.05$). The
        posterior probability that the release would break is $0.009/0.0585 = 0.15$.
...
\begin{ans}
  \item Distances from (50, 55): F 11.18 (Late), D 15.81 (Late), C 22.36 (On time),
        E 23.77, B 33.54, A 50.0. The three nearest vote Late, Late, On time:
        \textbf{Late}.
...
\begin{ans}
  \item Examples in the same cluster tend to share a label, so the decision boundary
        ...
        one cluster --- for example tickets with different root causes whose text and
        history look the same (a timeout caused by the network and one caused by a
        slow database query).
...
\begin{ans}
  ...
  \item $V^{\pi}(s)$ is the expected return from $s$ following $\pi$; $Q^{\pi}(s, a)$ is the
        ...
        $V^{\pi}(s) = Q^{\pi}(s, \pi(s))$.
  \item Because a team that keeps shipping on a debt-laden codebase eventually freezes
        it, and a frozen codebase delivers nothing for ever however hard the team
        ``ships''. The small immediate gain of shipping is outweighed by the lost future
        value that refactoring preserves.
\end{ans}
...
\begin{ans}
  \item For example: ``Over the next two sprints, the warning flags at least 70\% of
        the tasks that go on to finish late, and no more than one flag in three is a
