...
Suppose a project office asks you for a program that warns, at the start of a
sprint, which tasks will finish late. You could sit down and write rules:
\emph{if a task is 8 story points or more, and it waits on another team, and its
requirements changed this week, flag it.} The first version catches some late
tasks. Then the teams change, the tools change, a new client arrives --- and the
rules stop working. You add more. After a year you have two hundred rules that
contradict one another, nobody understands, and that still miss the task that
slipped this morning.

Now suppose instead that you collect five thousand completed tasks, each marked
on time or late, and hand them to a program whose job is to \emph{find} the
rules. When the situation changes you do not rewrite anything: you collect new
examples and let the program find new rules. That is the whole idea of machine
learning, and \dsref{program-vs-learn} shows it in one picture.
...
numerous to write (routing thousands of tickets a day to the right team); when
nobody can articulate them (judging from a ticket's wording how urgent it is);
when they change faster than a person can rewrite them (estimating effort while
the team and its tools change); and when they must be different for every case
(each team's own pace). If you can write the rule down in an afternoon and it
will not change, write it --- a rule is cheaper, faster and easier to explain than
any model.
\end{conceptbox}
...
\smallskip
\textbf{1. Late-task warning.}
\begin{tight}
  \item $T$: classify each task in the new sprint as likely late or on time.
  \item $P$: the fraction of late tasks flagged, with no more than one false alarm
        in every three flags.
  \item $E$: completed tasks from past sprints, each with its record and whether
        it finished late.
\end{tight}
...
\textbf{3. A scheduler learning to assign tickets.}
\begin{tight}
  \item $T$: choose which engineer receives each incoming support ticket.
  \item $P$: average time until tickets are resolved.
  \item $E$: the scheduler's own past assignments --- no labelled examples of the
        ``right'' engineer, only how each assignment turned out.
\end{tight}

\smallskip
\textbf{Notice the choice of $P$ in problem 1.} ``Accuracy'' would have been the
obvious answer and the wrong one: if 5\% of tasks run late, a model that says
``on time'' for every task is 95\% accurate and useless. Choosing $P$ is a design
decision with consequences, and \chref{evaluation} is devoted to it.
\end{worked}
...
Will this task finish late? & 6, 8--14 \\
Unsupervised & Features only & Groups, directions, maps & Kinds of project in a
portfolio & 15, 16 \\
Semi-supervised & Few labels, many unlabelled & A classifier using both &
Triage of ten thousand tickets when two hundred are labelled & 17 \\
Reinforcement & States, actions, rewards & A policy: what to do in each state &
When to pay down technical debt & 19, 20 \\
\bottomrule
\end{longtable}

\begin{alertbox}[Classification or Regression? Look at the Target, Not the Features]
The paradigm is decided by what $y$ is. ``Predict a project's final cost'' is
regression; ``predict whether the project will overrun its budget'' is
classification --- same projects, same features, different $y$. Numbers used as
category codes (team 7, office 3) are a common trap: ask whether averaging two
...
repeatedly: predicting, two weeks into a project, whether it will finish late.

\begin{worked}[The Four Design Decisions]
\textbf{1. Choose the training experience.} Records of past projects with their
data at Week~2 and whether they finished late. Is it representative? Only if past
projects resemble this one --- a new kind of client or a new technology stack
breaks that.

\textbf{2. Choose the target function.} What exactly should be learned?
\begin{tight}
  \item Option A: $V:\ \text{project} \to \{\text{on time}, \text{late}\}$.
  \item Option B: $V:\ \text{project} \to [0,1]$, the probability of finishing late.
\end{tight}
Option B is more useful: it lets the project office review the riskiest projects
first.

\textbf{3. Choose a representation} for the approximation $\hat{V}$ of the
target. For example, a weighted sum of features:
\[
\hat{V}(\mathbf{x}) = \sigma(w_0 + w_1\, \text{requirement changes} + w_2\,
\text{team experience} + w_3\, \text{tasks done by Week 2})
\]
...
A new team's first three stories took 2 hours (1 point), 4 hours (2 points) and
6 hours (3 points). How long will a 4-point story take? ``8 hours'' is the
natural answer, but the rule ``2 hours per point up to 3 points, and 0 above''
fits the three stories just as perfectly. Nothing in the data prefers one to the
other. You prefer 8 because you assume the simpler pattern continues. That
assumption --- the \term{inductive bias} --- is not a defect to be removed: a
learner with no bias at all cannot generalise beyond the examples it has seen.
\chref{concept} proves this, and every later chapter names the bias of its
algorithm.
\end{conceptbox}
...
layers. A scheduling tool that plans a project with task durations typed in by
the manager is AI but not ML; the same tool with durations learned from past
projects is both.
...
\begin{tight}
  ...
  \item \textbf{Treating a category code as a number.} Team 7 is not ``more''
        than team 3; averaging them means nothing.
...
Write a rule by hand, then let an algorithm learn one, and compare them on tasks
neither has seen. The lab makes its own data: 1{,}200 completed tasks from past
sprints, each with its size in story points, the number of other tasks it waited
on, its assignee's years of experience and the number of requirement changes,
and whether it finished late.
...
\end{lstlisting}
The hand-written rule does \emph{worse} than simply predicting ``on time'' for
every task; the tree beats both. Change the threshold 8 in the rule, and the
depth 3 of the tree, and watch the numbers. Which one is easier to improve by hand?
\end{lab}

%---------------------------------------------------------------------
\begin{checkpoint}
\begin{tightnum}
  \item A project tool marks a task ``at risk'' whenever its logged hours pass
        the estimate by half. Is this machine learning? Use the definition.
  \item Classify each as regression, classification, clustering or reinforcement
        learning: (a)~predicting next sprint's velocity in story points;
        (b)~grouping past projects with no labels; (c)~deciding whether a change
        request will be approved; (d)~a scheduler learning to order the backlog
        from the delivery times that follow.
...
\begin{reviewq}
  ...
  \item \textbf{(MCQ)} Ten thousand support tickets, of which 200 are labelled
        with their category, are data for: (a)~supervised (b)~unsupervised
        (c)~semi-supervised (d)~reinforcement learning.
  \item \textbf{(Short)} Give one project-management problem for which you would
        write rules rather than learn a model, and explain why.
  ...
  \item \textbf{(Applied)} A software company wants to suggest which developer
        should review each pull request. Specify $T$, $P$ and $E$; state which
        paradigm applies; and describe the four design decisions you would make.
  \item \textbf{(Applied)} For the late-task warning in the first worked example,
        describe what could make the training experience unrepresentative of the
        sprints the model will face next year, and what you would do about it.
\end{reviewq}
